def create_steering_control(bus, apply_steer, idx):
  ...
  return [0x180, 0, "".join(map(chr, dat)), bus]

def create_adas_keepalive(bus):
  dat = "\x00\x00\x00\x00\x00\x00\x00"
  return [[0x409, 0, dat, bus], [0x40a, 0, dat, bus]]

def create_gas_regen_command(bus, throttle, idx, acc_engaged, at_full_stop):
...
def create_friction_brake_command(bus, apply_brake, idx, near_stop, at_full_stop):
  if apply_brake == 0:
    mode = 0x10
  else:
    mode = 0xa0
